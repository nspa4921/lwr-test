...
function normalizeSpecifier(assetId, assetPathMap, resourcePaths) {
  ...
  if (type === "content-asset") {
    return (0, import_path.join)(resourcePaths.contentDir, specifier);
  }
  if (specifier[0] === "$") {
...
var LwrAssetRegistry = class {
  ...
  async getAsset(assetId, runtimeEnvironment) {
    const assetFilePath = normalizeSpecifier(assetId, this.assetPathMap, this.resourcePaths);
  ...
  getHashedAssetPath({ownHash, entry}, runtimeEnvironment) {
    ...
    return (0, import_path.join)(this.cacheDir, `${runtimeHash}_${ownHash}_${(0, import_path.basename)(entry)}`);
  }
